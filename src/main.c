...
void print_opt_solution(ll** S, int start, int end)
{
    if (start == end) printf("A_%d", start);
    else
    {
        printf("(");
        print_opt_solution(S, start, S[start][end]);
        print_opt_solution(S, S[start][end] + 1, end);
        printf(")");
    }
}
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        printf("Call: ./program_name infile_path");
        printf("File path:\n");
        scanf("%s", filepath);
    }
    filepath = argv[1];
    ...
    for (int i = 1; i < arr_len - 1; i++) sum += dim_array[i] * dim_array[i+1];
    printf("Broj mnozenja po redu je %lld\n", sum * dim_array[0]);
    printf("Optimalan poredak: ");
    print_opt_solution(Par_placement, 1, arr_len - 1);
    printf("\n");
